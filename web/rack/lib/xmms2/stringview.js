 ...
 * |*|
 * |*|  This framework is released under the GNU Lesser General Public License,
 * version 3 or later.
 ...
 * typedArray will be created. */
  ...
  if (nTranscrType < 8) {
    ...
    if (nOutptLen === 0 || nTranscrType < 4 && vSource.encoding === this.encoding && nCharStart === 0 && nOutptLen === nInptLen) {
      ...
      /* the encoding is the same, the length too and the offset is 0... or the
 * input is empty! */
 ...
 * */
      ...
      /* the source has a fixed-length encoding but the new StringView has a
 * variable-length encoding... */
      ...
      /* the source has a variable-length encoding but the new StringView has a
...
StringView.loadUTF8CharCode = function (aChars, nIdx) {
  ...
  return nPart > 251 && nPart < 254 && nIdx + 5 < nLen ?
      ...
      /* six bytes */ (nPart - 252) * 1073741824 + (aChars[nIdx + 1] - 128 <<
24) + (aChars[nIdx + 2] - 128 << 18) + (aChars[nIdx + 3] - 128 << 12) + (aChars[nIdx + 4] - 128 << 6) + aChars[nIdx + 5] - 128
    : nPart > 247 && nPart < 252 && nIdx + 4 < nLen ?
      /* five bytes */ (nPart - 248 << 24) + (aChars[nIdx + 1] - 128 << 18) +
(aChars[nIdx + 2] - 128 << 12) + (aChars[nIdx + 3] - 128 << 6) + aChars[nIdx + 4] - 128
