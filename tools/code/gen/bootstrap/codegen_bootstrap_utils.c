...
void slashdotscore(char *out, char *in) {
  char *slash;
  char *dot;
  strcpy(out, in);
  slash = NULL;
  dot = NULL;
  while ((slash = strstr(out, "/")) || (dot = strstr(out, "."))) {
    if (slash)
      slash[0] = '_';
    if (dot)
      dot[0] = '_';
...
static int codegen_enum(header_data *hdata, int n, char *prefix,
 char *suffix, FILE *out) {
  ...
  for (i = 0; i < n; i++) {
    for (j = 0; j < hdata[i].def_count; j++) {
      if (is_prefix(hdata[i].defs[j].info.name,prefix)
        && is_suffix(hdata[i].defs[j].info.name,suffix) && (hdata[i].target_count > 0)) {
        fprintf(out,"  ");
