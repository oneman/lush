...
static void krad_xmms_playback_cmd_internal (krad_xmms_t *krad_xmms, krad_xmms_playback_cmd_t cmd);
...
static int krad_xmms_media_info_callback (xmmsv_t *value, void *userdata) {
  ...
  if (krad_xmms->tags != NULL) {
    kr_tags_set_tag_internal(krad_xmms->tags, "artist", krad_xmms->artist);
    kr_tags_set_tag_internal(krad_xmms->tags, "title", krad_xmms->title);
    kr_tags_set_tag_internal(krad_xmms->tags, "now_playing", krad_xmms->now_playing);
  }
...
static int krad_xmms_playtime_callback (xmmsv_t *value, void *userdata) {
  ...
  if (krad_xmms->playtime != (playtime / 1000)) {
    ...
    if (krad_xmms->tags != NULL) {
      kr_tags_set_tag_internal(krad_xmms->tags, "playtime", krad_xmms->playtime_string);
    }
  }
...
static int krad_xmms_playback_status_callback (xmmsv_t *value, void *userdata) {
  ...
  if ((krad_xmms->tags != NULL) && (krad_xmms->playback_status == 0)) {
    kr_tags_set_tag_internal(krad_xmms->tags, "playtime", "0:00");
  }
...
static void krad_xmms_error_state (krad_xmms_t *krad_xmms) {
    ...
    kr_tags_set_tag_internal(krad_xmms->tags, "playtime", "0:00");
    kr_tags_set_tag_internal(krad_xmms->tags, "artist", "Error");
    kr_tags_set_tag_internal(krad_xmms->tags, "title", "Error");
    kr_tags_set_tag_internal(krad_xmms->tags, "now_playing", "Error - Error");
}
...
static void krad_xmms_disconnect (krad_xmms_t *krad_xmms) {
  ...
  if ((krad_xmms->tags != NULL) && (krad_xmms->destroying == 0)) {
      ...
      if (pollfds[n].revents) {
        if (pollfds[n].fd == krad_xmms->handler_thread_socketpair[1]) {
          if (pollfds[n].revents & POLLIN) {
            ret = read (krad_xmms->handler_thread_socketpair[1], &byte, 1);
            if (ret != 1) {
              printke ("krad xmms2: unexpected read return value %d in handle thread", ret);
            } else {
              if (byte == 0) {
                krad_xmms->handler_running = 2;
                return;
                ...
                cmd = byte;
                krad_xmms_playback_cmd_internal (krad_xmms, cmd);
              }
            }
          }
        } else {
...
static void krad_xmms_start_handler (krad_xmms_t *krad_xmms) {
  ...
  if (socketpair(AF_UNIX, SOCK_STREAM, 0, krad_xmms->handler_thread_socketpair)) {
    ret = strerror_r (errno, error_string, sizeof(error_string));
    if (!ret) return;
...
static void krad_xmms_stop_handler (krad_xmms_t *krad_xmms) {
  ...
  if (krad_xmms->handler_running == 1) {
    ret = write (krad_xmms->handler_thread_socketpair[0], &byte, 1);
    if (ret != 1) {
      printke ("krad xmms2: unexpected write return value %d", ret);
    }
  }
  if (krad_xmms->handler_running > 0) {
...
void krad_xmms_destroy (krad_xmms_t *krad_xmms) {
  ...
  if (krad_xmms->tags != NULL) {
    kr_tags_set_tag_internal(krad_xmms->tags, "playtime", "");
    kr_tags_set_tag_internal(krad_xmms->tags, "artist", "");
    kr_tags_set_tag_internal(krad_xmms->tags, "title", "");
    kr_tags_set_tag_internal(krad_xmms->tags, "now_playing", "");
  }
  free(krad_xmms);
}
