...
int32_t kr_perspective_apply(kr_perspective *p, kr_image *out, kr_image *in) {
  ...
  if ((out->px[0] == NULL) || (in->px[0] == NULL)) {
    return -2;
  }
  if ((in->info.w != p->width) || (in->info.h != p->height)) {
    return -3;
  }
  if ((out->info.w != p->width) || (out->info.h != p->height)) {
    return -4;
  }
  return kr_perspective_argb(p, out->px[0], in->px[0]);
}
