#if !defined(_audio_biquad_H)
# define _audio_biquad_H (1)
...
static inline float biquad_resp(biquad *f, float fc, float fs);
static inline float biquad_resp(biquad *f, float fc, float fs) {
  float w;
  float numerator;
  float denominator;
  float magnitude;
  w = 2.0 * M_PI * fc / fs;
  numerator = f->b0 * f->b0 + f->b1 * f->b1
   + f->b2 * f->b2 + 2.0 * (f->b0 * f->b1 + f->b1 * f->b2) * cosf(w)
   + 2.0 * f->b0 * f->b2 * cosf(2.0 * w);
  denominator = 1.0 + f->a1 * f->a1 + f->a2 * f->a2
   + 2.0 * (f->a1 + f->a1 * f->a2) * cosf(w)
   + 2.0 * f->a2 * cosf(2.0 * w);
  magnitude = sqrt(numerator / denominator);
  return 20 * logf(magnitude) / logf(10);
}
