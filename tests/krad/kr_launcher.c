...
typedef struct kr_launcher kr_launcher;

struct kr_launcher {
  int sd;
};

int kr_launcher_destroy(kr_launcher **launcher);
...
void kr_launcher_loop(kr_launcher *launcher) {
  int32_t ret;
  char input[1024];
  struct pollfd lsd[1];
...
int32_t kr_launcher_destroy(kr_launcher **launcher) {
...
void kr_launcher_start(kr_launcher *controller, kr_launcher *launcher) {
  pid_t sid;
  pid_t pid;
  int32_t flags;
  /* FILE *refp; */
...
kr_launcher *kr_launcher_create() {
  kr_launcher *launcher;
  kr_launcher *launcher_controller;
  int sockets[2];
  if (socketpair(AF_UNIX, SOCK_STREAM, 0, sockets)) {
    fprintf (stderr, "Krad System: can't socketpair errno: %d", errno);
    return NULL;
  }
  launcher = calloc(1, sizeof(kr_launcher));
  launcher_controller = calloc(1, sizeof(kr_launcher));
...
void kr_launcher_launch(kr_launcher *launcher, char *cmd[]) {
  int i;
  int pos;
  char output[512];
  pos = 0;
  for (i = 0; cmd[i] != NULL; i++) {
    pos += snprintf(output + pos, (sizeof(output) - 2) - pos, "%s,", cmd[i]);
