...
int cpu_meter_render_callback (void *pointer, uint32_t time) {
  ...
  cpu_meter->current_cpu_usage = krad_app_get_cpu_usage ();
...
static void show_cpu_meter_window (int width, int height) {
  ...
  cpu_meter->cpu_usage = krad_app_get_cpu_usage ();
...
int main (int argc, char *argv[]) {
  ...
  krad_app_init ();
  krad_app_set_monitor_cpu_interval (800);
  krad_app_monitor_cpu_on ();
