...
tunit *tunit_gen(kr_graph *graph, char *name, int type, int from, int to, 
 tunit **units) {
  tunit_setup tunit_setup;
  tunit *unit;
  memset(&tunit_setup, 0, sizeof(tunit_setup));
  tunit_setup.graph = graph;
  tunit_setup.name = name;
  tunit_setup.integer = randr(0, 10);
  tunit_setup.type = type;
  if (type == TU_LINK) {
    tunit_setup.from = units[from];
    tunit_setup.to = units[to];
  }
  unit = tunit_create(&tunit_setup);
  if (unit && type == TU_LINK) {
    printf("new %s %s from %s to %s\n",tunit_type_to_str(unit->type),
...
int tunits_populate_graph(kr_graph *graph, tunit **units, int max) {
  tunit *tunit;
  tunit = tunit_gen(graph, "XMMS2", TU_SOURCE, 0, 0, NULL);
  units[0] = tunit;

  tunit = tunit_gen(graph, "Speakers", TU_OUTPUT, 0, 0, NULL);
  units[1] = tunit;

  units[2] = tunit_gen(graph, "XMMS2-Speakers", TU_LINK, 0, 1, units);
...
int run_test() {
  ...
  n = tunits_populate_graph(graph, units, MAX_TUNITS);
  for (i = 0; i < n; i++) {
