...
static int theora_enc_test(char *outfile, char *infile) {
  int i;
  ssize_t ret;
  int fd;
  uint8_t *buf;
  ...
  kr_codec_header theora_headers;
  ...
  kr_frame out_frame;
  kr_convert conv;
  kr_theora_encoder *enc;
  kr_theora_encoder_setup setup;
  memset(&in_frame, 0, sizeof(in_frame));
  memset(&out_frame, 0, sizeof(out_frame));
  memset(&od, 0, sizeof(od));
  memset(&theora_headers, 0, sizeof(theora_headers));
  ...
  raw_yuv.info.fmt = KR_PIXELS_YUV444;
  ...
  setup.mem = kr_alloc(kr_theora_encoder_sizeof());
  ...
  setup.info.eos = 0;
  enc = kr_theora_encoder_init(&setup);
  if (enc == NULL) return -1;
  in_frame.image = raw_yuv;
  in_frame.type = KR_IMAGE_RAW;
  ret = kr_theora_encoder_get_headers(enc, &theora_headers);
  if (ret < 0) return -1;
  ogg_setup.tracks = 1;
  ogg_setup.mem = kr_alloc(kr_ogg_muxer_sizeof());
  ogg_setup.header[0] = theora_headers;
  ...
  ret = kr_theora_encode(enc, &out_frame, &in_frame);
  if (ret != 0) {
...
static void print_usage() {
  printk("  ./theora_enc_test in.png out.ogg");
}
...
int main(int argc, char *argv[]) {
  ...
  ret = theora_enc_test(argv[2], argv[1]);
  if (ret) {
    printk("Failure!");
  }
  return ret;
